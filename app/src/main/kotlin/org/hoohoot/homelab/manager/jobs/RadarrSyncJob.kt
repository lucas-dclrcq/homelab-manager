package org.hoohoot.homelab.manager.jobs

import io.quarkus.logging.Log
import io.quarkus.scheduler.Scheduled
import jakarta.enterprise.context.ApplicationScoped
import jakarta.enterprise.context.control.ActivateRequestContext
import org.eclipse.microprofile.config.inject.ConfigProperty
import org.hoohoot.homelab.manager.library.infra.StatsSyncService

@ApplicationScoped
class RadarrSyncJob(
    private val statsSyncService: StatsSyncService,
    private val jobRunner: JobRunner,
    @param:ConfigProperty(name = "radarr-sync.every") private val every: String,
) : ManagedJob {
    override val identity = IDENTITY
    override val displayName = "Synchronisation des stats Radarr"
    override val schedule get() = "every $every"

    override suspend fun execute() {
        statsSyncService.syncRadarr()
    }

    @Scheduled(
        identity = IDENTITY,
        every = "{radarr-sync.every}",
        delayed = "{stats-sync.initial-delay}",
        concurrentExecution = Scheduled.ConcurrentExecution.SKIP,
    )
    @ActivateRequestContext
    suspend fun run() {
        Log.info("Running Radarr stats sync")
        jobRunner.runScheduled(this)
    }

    companion object {
        const val IDENTITY = "radarr-sync"
    }
}
