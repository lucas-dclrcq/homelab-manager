...
@ApplicationScoped
class JellystatService(@param:RestClient private val jellystatRestClient: JellystatRestClient) : ViewingStats {

    override suspend fun topMovies(lastDays: Int, limit: Int): List<MostPopularMedia> =
        getMostPopularByType(lastDays, JellystatMediaType.Movie)
    ...
    override suspend fun topSeries(lastDays: Int, limit: Int): List<MostPopularMedia> =
        getMostPopularByType(lastDays, JellystatMediaType.Series)
            .map { MostPopularMedia(it.name, it.uniqueViewers) }
            .sortedByDescending { it.uniqueViewers }
            .take(limit)
