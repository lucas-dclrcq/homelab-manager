...
class ApplicationsResource(
    ...
    suspend fun listApplications(): List<ApplicationDto> =
        applicationRepository.listSummaries().map {
            ApplicationDto(it.id, it.name, it.category, it.description, it.url, it.requiresVpn, it.hasLogo)
        }
    ...
    suspend fun createApplication(
        ...
        return Response.status(Response.Status.CREATED)
            .entity(
                ApplicationDto(
                    saved.id!!, saved.name, saved.category, saved.description,
                    saved.url, saved.requiresVpn, saved.logo != null
                )
            )
            .build()
    }
