...
<template>
  <BaseCard :accent="accent">
    <div class="flex items-start gap-4">
      ...
      <div
        v-else
        class="flex size-12 shrink-0 items-center justify-center rounded-xl text-xl font-bold text-white"
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2">
          <h3 class="truncate font-semibold text-slate-100">{{ application.name }}</h3>
          <BaseBadge v-if="application.requiresVpn" color="rose">
            <PadlockIcon />
            VPN
          </BaseBadge>
        </div>
        <p class="mt-1 line-clamp-2 text-sm text-slate-400">{{ application.description }}</p>
        <a
          :href="application.url"
          target="_blank"
          rel="noopener noreferrer"
          class="mt-3 inline-flex items-center gap-1 text-sm font-medium transition-colors hover:underline"
          :style="{ color: accent }"
        >
          Ouvrir ↗
        </a>
